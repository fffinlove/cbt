...
func Mint(to interop.Hash160, id []byte, name, image, atk, def, hp string) bool {
	...
	b := std.JSONSerialize(m)
	storage.Put(ctx, mkTokenKey(id), b)

	total := getBalanceOf(ctx, mkBalanceKey(to, id))
	total++
	addToBalance(ctx, to, id, total)
	addOwner(ctx, id, to)
	postTransfer(contractOwner, to, id, 1, b)
...
func Properties(id []byte) map[string]string {
	...
	m := std.JSONDeserialize(storage.Get(ctx, mkTokenKey(id)).([]byte)).(map[string]string)
	return m
}
...
func OnNEP17Payment(from interop.Hash160, amount int, data any) {
	...
	m := std.JSONDeserialize(data.([]byte)).(map[string]string)
	m["Owner"] = address.FromHash160(from)
	id := []byte(m["ID"])
