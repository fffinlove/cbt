...
var (
	...
	contractOwner = address.ToHash160("NYE7BDVfMNiGuNR1KLdRqtV3ryuczGhwJg")
)
...
// Decimals returns token decimals, this NFT is non-divisible.
func Decimals() int {
	return decimals
}
...
func Mint(to interop.Hash160, id []byte, name, image, atk, def, hp string) bool {
	if !Verify() {
		return false
	}
	...
	m := map[string]string{
		...
		"Name":    name,
		"Image":   image,
		"Owner":   address.FromHash160(to),
		"ATK":     atk,
		"DEF":     def,
		"HP":      hp,
	}

	data := std.JSONSerialize(m)
	total := getBalanceOf(ctx, mkBalanceKey(to, id))
	total++
	addToBalance(ctx, to, id, total)
	addOwner(ctx, id, to)
	postTransfer(contractOwner, to, id, 1, data)
	storage.Put(ctx, mkTokenKey(id), data)

	totalSupply := totalSupply(ctx)
	totalSupply++
	storage.Put(ctx, []byte(totalSupplyPrefix), totalSupply)
	return true
}
...
func Properties(id []byte) map[string]string {
	...
	key := mkTokenKey(id)
	m := std.JSONDeserialize(storage.Get(ctx, key).([]byte)).(map[string]string)
	return m
}
...
func OnNEP17Payment(from interop.Hash160, amount int, data any) {
	...
	if amount < 1_00000000 {
		panic("minting NFSO costs at least 1 GAS")
	}

	nft := data.(map[string]string)
	id := []byte(nft["Id"])
	nft["Owner"] = address.FromHash160(from)
	...
	key := mkTokenKey(id)
	storage.Put(ctx, key, nft)

	total := totalSupply(ctx)

	addOwner(ctx, id, from)
	addToBalance(ctx, from, id, amount)

	total++
	storage.Put(ctx, []byte(totalSupplyPrefix), total)
	data = std.JSONSerialize(nft)
	postTransfer(nil, from, id, amount, data) // no `data` during minting
}
